...
public class Elevator {
    ...
    boolean elevatorReadyToExit(int floor) {
        return this.doors == 1
                && Math.round(10.0D * this.currentPosition) == (long) (10 * floor * this.model.building.floorHeight);
    }

    boolean elevatorReadyToBoard(int departFloor, int targetFloor) {
        return this.doors == 1
                & Math.round(
                        10.0D * this.currentPosition) == (long) (10 * departFloor * this.model.building.floorHeight)
                & this.passengerCount < this.model.building.elevatorCapacity & this.servicesFloors[targetFloor]
                & (targetFloor > departFloor && this.committedDirection != 1
                        || targetFloor < departFloor && this.committedDirection != 0);
    }
    ...
    int tick() {
        ...
        if (Math.round(10.0D * this.targetPosition) != Math.round(10.0D * this.currentPosition)) {
            if (this.doors == 1) {
                this.slowing = false;
                if (this.model.clock >= 5L + this.lastBoarded
                        & (!this.beamBroken | this.passengerCount >= this.model.building.elevatorCapacity)) {
                    this.doors = 4;
                    this.doorMoving = 10;
                    if (Challenge.logging >= 3) {
                        System.out.println("Elevators doors on "
                                + Math.round(this.currentPosition) / (long) this.model.building.floorHeight + " closing"
                                + " at t=" + this.model.clock);
                    }
                ...
                if (Challenge.logging >= 3) {
                    System.out.println("Targetting " + this.targetPosition + ". " + "Position: " + this.currentPosition
                            + " with speed: " + this.currentSpeed + " at: " + this.model.clock);
                }
                ...
                if (Math.round(10.0D * this.currentSpeed) != 0L
                        & (Math.round(10.0D * this.currentPosition) > Math.round(10.0D * this.targetPosition)
                                & this.currentDirection == 0
                                | Math.round(10.0D * this.currentPosition) < Math.round(10.0D * this.targetPosition)
                                        & this.currentDirection == 1)) {
                    this.slowing = true;
                ...
                if (Math.round(this.currentPosition * 10.0D) > (long) (10
                        * ((this.model.building.floors - 1) * this.model.building.floorHeight + 1))) {
                    if (Challenge.logging >= 1) {
                        System.out.println("Elevator going out of bounds at t=" + this.model.clock);
                    }

                    this.currentPosition = (double) (this.model.building.floors - 1)
                            * (double) this.model.building.floorHeight;
                    this.currentSpeed = 0.0D;
                }

                if (this.currentDirection == 2 && Challenge.logging >= 1) {
                    System.out
                            .println("EEEEEK - Elevator stopped when it should be up or down at t=" + this.model.clock);
                }
            }
        } else {
            if (this.doors == 2) {
                ...
                if (Challenge.logging >= 3) {
                    System.out.println("Elevators doors on "
                            + Math.round(this.currentPosition) / (long) this.model.building.floorHeight + " openning"
                            + " at t=" + this.model.clock);
                }
        ...
        if (Challenge.logging >= 3) {
            System.out.println("Speed=" + this.currentSpeed + ". Position: " + this.currentPosition + ". Speed/Accel: "
                    + tickSpeed + "/" + tickAccel);
        }
