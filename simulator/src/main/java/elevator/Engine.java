...
class Engine {
    ...
    void clearPassengers() {
        if (!this.model.passengers.isEmpty()) {
            for (int i = this.model.passengers.size() - 1; i >= 0; --i) {
                ...
                if (p.boardTime == 0L) {
                    p.boardTime = (long) Math
                            .round(Math.max((float) (this.clock - p.createTime), (float) this.averageWait))
                            + p.createTime;
                }

                p.departTime = (long) ((int) Math.round(
                        Math.max((double) ((float) (this.clock - p.boardTime)), this.averageTravel - this.averageWait)))
                        + p.boardTime;
                if (Challenge.logging >= 2) {
                    System.out.println("Remaining passenger " + p.id + " removal : " + p.createTime + ", " + p.boardTime
                            + ", " + p.departTime);
                }
    ...
    void updateClockText() {
        this.simClocks[0] = " Clock: " + this.nf2.format(this.clock / 36000L) + ":"
                + this.nf2.format(this.clock / 600L % 60L) + ":" + this.nf2.format(this.clock / 10L % 60L) + "."
                + this.nf1.format(this.clock % 10L) + " ";
        this.simClocks[1] = " Repeat: " + (this.currentRepeat + 1) + " of " + this.model.getRepeat() + " | "
                + this.nf2.format((this.clock - this.repeatStartTime) / 36000L) + ":"
                + this.nf2.format((this.clock - this.repeatStartTime) / 600L % 60L) + ":"
                + this.nf2.format((this.clock - this.repeatStartTime) / 10L % 60L) + "."
                + this.nf1.format((this.clock - this.repeatStartTime) % 10L) + " ";
        this.simClocks[2] = " Stage: " + (this.currentStage + 1) + " of " + this.model.getStageNum() + " | "
                + this.nf2.format((this.clock - this.stageStartTime) / 36000L) + ":"
                + this.nf2.format((this.clock - this.stageStartTime) / 600L % 60L) + ":"
                + this.nf2.format((this.clock - this.stageStartTime) / 10L % 60L) + "."
                + this.nf1.format((this.clock - this.stageStartTime) % 10L) + " ";
    }
    ...
    private void updatePassengers() {
        ...
        for (p = 0; p < this.floors; ++p) {
            ...
            if (arrivalRate > 0.0D & Math.random() <= arrivalRate) {
                ...
                for (; !foundElevator & elevatorTest < this.elevators; ++elevatorTest) {
                    if (this.model.building.elevator[elevatorTest].servicesFloors[p]
                            & this.model.building.elevator[elevatorTest].servicesFloors[var14]) {
                        foundElevator = true;
                    }
                }
                ...
                if (!foundElevator) {
                    ...
                    for (int var19 = 0; var19 < this.floors; ++var19) {
                        ...
                        for (elevatorTest = 0; elevatorTest < this.elevators; ++elevatorTest) {
                            if (this.model.building.elevator[elevatorTest].servicesFloors[p]
                                    & this.model.building.elevator[elevatorTest].servicesFloors[var19]) {
                                foundDepart = true;
                            }

                            if (this.model.building.elevator[elevatorTest].servicesFloors[var14]
                                    & this.model.building.elevator[elevatorTest].servicesFloors[var19]) {
                                foundTarget = true;
                            }
                        }
                ...
                if (Challenge.logging >= 2) {
                    System.out.println(
                            "Adding passenger leaving from " + p + " going to " + var14 + " at t=" + this.clock);
                }
            }
        }
        ...
        if (!this.model.passengers.isEmpty()) {
            Iterator<Passenger> iter = this.model.passengers.iterator();
    ...
    private void trackResults(Passenger p) {
        ...
        this.averageWait = this.averageWait
                * (((double) this.passengersProcessed - 1.0D) / (double) this.passengersProcessed)
                + (double) (p.boardTime - p.createTime) / (double) this.passengersProcessed;
        this.averageTravel = this.averageTravel
                * (((double) this.passengersProcessed - 1.0D) / (double) this.passengersProcessed)
                + (double) (p.departTime - p.createTime) / (double) this.passengersProcessed;
    }
}
